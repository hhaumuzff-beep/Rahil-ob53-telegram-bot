...
import logging
import sys
from datetime import datetime, timedelta, timezone  # ✅ ADDED: timezone for fixing warning
...
def reset_limits():
    ...
    while True:
        try:
            # ✅ FIXED: Removed utcnow() warning by using timezone.utc
...
# === TOKEN AUTO UPDATE ===
# ✅ FIXED: Wrapped in try-except so app doesn't crash if token_refresh.py is missing
try:
...
def process_like(message, region, uid):
    user_id = message.from_user.id
    now_utc = datetime.now(timezone.utc)  # ✅ FIXED: timezone.utc
